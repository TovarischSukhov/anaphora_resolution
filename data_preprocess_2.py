...
print(sum(tokens['anaph'].loc[tokens['anaph']==1]))
...
def add_anaphora(i, row, anaphora):
    counter_words = 0
    # print(i, row)
    anaphora_local = pd.DataFrame({"target": [], "t_gend": [], "t_count": [], 't_vec-3': [], 't_vec-5': [],
                                   't_vec+3': [], 't_vec+5': [], 'discourse': [],
                                   "cand_vec": [], "c_gend": [], "c_count": [], 'c_pow': [], 'c_vec-3': [],
                                   'c_vec-5': [],
                                   'c_vec+3': [], 'c_vec+5': [],
                                   'dif_vec-3': [], 'dif_vec-5': [], 'dif_vec+3': [], 'dif_vec+5': [],
                                   'dist': [], 'dif_cand_disc': [], 'answ': []})
    ...
    for col in vec_cls:
        # print('step7,5')
        anaphora_local[col] = anaphora_local[col].astype('object')
    ...
    while number_added <= 20:
        ...
        if tokens.iloc[current_posit]['lemma'] not in punctuation:
            ...
            # print('STP8')
            anaphora_local.loc[len(anaphora) + 1] = [answ, cand_count, cand_gend, cand_pow, c_vec_plus3, c_vec_plus5,
                                                     c_vec_minus3,
                                                     c_vec_minus5, candidate_vec, dif_cand_disc, dif_vec_plus3,
                                                     dif_vec_plus5,
                                                     dif_vec_minus3, dif_vec_minus5, discource, dist, target_count,
                                                     target_gend,
                                                     t_vec_plus3, t_vec_plus5, t_vec_minus3, t_vec_minus5, target]

    if found_answ:
        # print('WARNING: answ_not_found for ', target, shift, i)
        anaphora = pd.concat(anaphora, anaphora_local)
    else:
        not_found.append([target, shift, i])
    return anaphora


def add_anaphora_old(i, row, anaphora):
    counter_words = 0
    # print(i, row)
    shift = row['shift']
    target_link = row['link']
...
for i, row in tqdm(tokens.loc[tokens['anaph']==1].iterrows()):
    counter_words = 0;
    #print(i)
    i = int(i)
    if row['anaph'] == 1 and row['link'] != 0:
        print('yay')
        anaphora = add_anaphora_old(i, row, anaphora)
# print(anaphora.head(40))
# print(add_anaphora(68, tokens.iloc[68], anaphora))

anaphora.to_csv('anaphora_no_null.csv')
